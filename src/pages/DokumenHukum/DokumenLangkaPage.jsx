...
const DokumenLangkaPage = () => {
  ...
  const [title, setTitle] = useState("Dokumen Langka");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const mappedDocs = await Promise.all(
        rawDocs.map(async (item) => {
          ...
          return {
            ...
            status: getField("Subjek Dokumen Langka"),
      ...
      setTitle("Dokumen Langka");
      setTotalItems(result.data?.pagination?.total || 0);
    } catch (error) {
      console.error("Error fetching dokumen langka data:", error);
    } finally {
      setIsLoading(false);
    }
  };
            ...
            {documents.length > 0 ? (
              documents.map((doc) => (
                <DocCard
                  ...
                  image={doc.image}
                  onDetailClick={() =>
                    navigate(`/dokumentasi/dokumen-langka/${doc.slug}`)
                  }
                />
              ))
            ) : (
