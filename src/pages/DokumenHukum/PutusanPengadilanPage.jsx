...
const PutusanPengadilanPage = () => {
  ...
  const [title, setTitle] = useState("Dokumen Putusan Pengadilan");
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      const mappedDocs = await Promise.all(
        rawDocs.map(async (item) => {
          ...
          return {
            ...
            status: getField("Subjek Putusan Pengadilan"),
      ...
      setTitle("Dokumen Putusan Pengadilan");
      setTotalItems(result.data?.pagination?.total || 0);
    } catch (error) {
      console.error("Error fetching putusan pengadilan data:", error);
    } finally {
      setIsLoading(false);
    }
  };
            ...
            {documents.length > 0 ? (
              documents.map((doc) => (
                <DocCard
                  ...
                  image={doc.image}
                  onDetailClick={() =>
                    navigate(`/dokumentasi/putusan-pengadilan/${doc.slug}`)
                  }
                />
              ))
            ) : (
